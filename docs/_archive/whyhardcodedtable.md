...
- **Scenario B (With Demo Data):** A visitor arrives. They _instantly_ see a beautiful, populated table showing exactly what the app does. Then, if the backend is ready, it seamlessly swaps to live data. **Result:** Instant engagement.
...
- **Empty Data:** If the database is empty (new installation), showing a blank table with no columns looks broken. Showing demo data guides the user on what _will_ be there.
...
3.  **Logic Gaps:** The `fetch` logic relies on `usingRealData` flag. If the fetch fails, the code was designed to _keep_ the existing data (which was the demo data). If you removed it, the fallback became `undefined` or empty, breaking the UI.
...
3.  **Update HTML:**
    ...
    {#if loading}
    	...
    	{#each Array(5) as _}
    		<div class="mb-2 h-10 animate-pulse rounded bg-slate-200"></div>
    	{/each}
...
Fetch the data on the server _before_ the page loads. The user never sees a loading state.
...
- **Dashboard (Product):** Use **Empty States**. If a teacher logs in and sees "Ahmad Santoso" (who doesn't exist), they will panic and think the database is corrupted.
  - _Correct Dashboard Behavior:_ Show the headers ("Nama", "NISN") but leave the rows empty with a message: _"Belum ada data siswa. Silakan tambah siswa baru."_
