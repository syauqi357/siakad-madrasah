...
<div class="relative w-full">
	...
	/>
	<label
		class="absolute top-2.5 left-2.5 z-10 cursor-text bg-white px-1 text-sm text-slate-400 transition-all duration-300 peer-placeholder-shown:top-3 peer-placeholder-shown:text-sm peer-focus:-top-2 peer-focus:text-xs peer-focus:text-blue-500"
		...
		>Name</label
	>
</div>
...
1.  **Consistency:** Every phone input in the app looks exactly the same. If we decide to change the border color to purple, we change it in _one file_, and it updates everywhere.
2.  **Readability:** The parent form file focuses on _structure_ (which fields go where) rather than _style_ (how many pixels of padding).
