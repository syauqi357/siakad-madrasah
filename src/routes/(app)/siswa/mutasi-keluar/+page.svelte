...
<div class="mx-0 md:mx-30">
	<div class="flex flex-col gap-6 p-4 md:p-8">
		...
		<button
			on:click={() => goto('/siswa')}
			class="flex w-fit items-center gap-2 rounded-full border bg-blue-500 px-4 py-2 text-sm text-blue-50 capitalize transition-all ease-in-out hover:gap-4 hover:bg-blue-700"
		>
			<ArrowLeft />kembali
		</button>
		<!-- Header -->
		<div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
			<div class="flex items-center gap-4">
				<div>
					<p class="text-3xl font-bold text-blue-700">Data Siswa Mutasi</p>
					<p class="text-sm text-gray-600">Daftar siswa yang telah keluar/pindah</p>
				</div>
			</div>
						...
						<tbody class="divide-y divide-gray-400">
							{#each filteredStudents as student, i (student.id)}
								<tr class="transition-colors hover:bg-gray-100">
									...
									<td class="px-4 py-5">
										<span
											class="rounded-md border border-amber-500 bg-yellow-300 px-2 py-1 text-xs font-medium text-yellow-800"
										>
											{getMutasiTypeLabel(student.mutasiType)}
										</span>
									</td>
									<td class="px-4 py-5">{formatDate(student.completionDate)}</td>
									<td class="px-4 py-5">{student.destinationSchool || '-'}</td>
									<td class="max-w-xs truncate px-4 py-5" title={student.reason}>
										{student.reason || '-'}
									</td>
								</tr>
							{/each}
