		...
		try {
			...
			if (result.assessmentTypes?.length > 0) {
				tableHeaders = [
					'Nama Siswa',
					'NISN',
					...result.assessmentTypes.map((tablesType: any) => tablesType.code)
				];
			} else {
...
<div class="min-h-screen bg-slate-50">
	...
	<div class="px-4 pt-16 pb-12 sm:px-6 sm:pt-24 sm:pb-16">
		<div class="mx-auto max-w-5xl text-center">
			<p class="text-sm font-medium tracking-wide text-blue-600 uppercase">
			...
			<p class="mx-auto mt-4 max-w-xl text-base text-slate-500 sm:text-lg">
				Sistem informasi akademik terpadu untuk pengelolaan data siswa, nilai, dan administrasi
				sekolah.
			</p>
	...
	<div class="mx-auto max-w-5xl px-4 pb-12 sm:px-6 sm:pb-16">
		<div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
			...
			<div
				class="row-span-2 flex flex-col justify-between rounded-lg border border-slate-200 bg-white p-6"
			>
				<div>
			...
			<div class="rounded-lg border border-slate-200 bg-white p-6">
				...
				<p class="mt-1.5 text-sm text-slate-500">
					Input nilai harian, UTS, UAS dengan sistem assessment fleksibel.
				</p>
			</div>
			...
			<div class="rounded-lg border border-slate-200 bg-white p-6">
				...
				<p class="mt-1.5 text-sm text-slate-500">
					Atur kelas, mata pelajaran, dan jadwal pengajaran.
				</p>
			</div>

			<!-- Card 4: Dokumentasi (wide) -->
			<div
				class="rounded-lg border border-slate-200 bg-linear-to-br from-slate-800 to-slate-900 p-6 sm:col-span-2"
			>
			...
			<div class="rounded-lg border border-slate-200 bg-white p-6">
				...
				<p class="mt-1.5 text-sm text-slate-500">
					Cetak rapor dan rekap nilai per kelas atau per siswa.
				</p>
			</div>
			...
			<div class="rounded-lg border border-slate-200 bg-white p-6 lg:col-span-1">
				...
				<p class="mt-1.5 text-sm text-slate-500">
					Kode sumber terbuka, bisa dikembangkan bersama komunitas.
				</p>
			</div>
		</div>
	</div>
	...
	<div class="mx-auto max-w-5xl px-4 pb-16 sm:px-6 sm:pb-24">
		...
		{#if error}
			<div
				class="mt-4 rounded-lg border border-red-200 bg-red-50 p-4 text-center text-sm text-red-600"
			>
				{error}
			</div>
		{/if}
