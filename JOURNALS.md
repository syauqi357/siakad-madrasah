...
This research has demonstrated that Personal Extreme Programming, when paired with an offline-first desktop deployment model, can deliver a maintainable academic information system suited to the operational constraints of Indonesian madrasahs. The implemented SIAKAD Madrasah system covers the full administrative surface required by the case institution — student, parent, teacher, class group, score, academic year, curriculum, school profile, and audit-log management,through a SvelteKit frontend, an Express.js backend organized in a layered (route–controller–service) architecture, a SQLite database accessed via Drizzle ORM, and an Electron-packaged desktop distribution.

Three findings stand out. First, the iteration history of 491 commits across 12 dynamically-scoped iterations provides empirical evidence that PXP supports adaptive solo development, with iteration scope tracking feature complexity rather than fixed sprint length. Second, white-box testing on memory-intensive Excel processing functions yielded 11 of 11 passing cases, confirming the reliability of the most data-intensive paths in the system. Third, two cycles of user feedback drove demonstrable improvements: the addition of usability hints on the bulk-upload feature and the migration of the distribution mechanism from a manually-executed batch file to an NSIS installer, raising the estimated installation success rate from approximately 30% to 95%.

Several directions remain open for future work. The persistence layer can be migrated from SQLite to a server-grade database such as PostgreSQL or MySQL through Prisma ORM in order to support multi-school deployments and shared reporting. A formal usability study using validated instruments (e.g., SUS or UEQ) would replace the qualitative usability evidence currently obtained through interviews. End-to-end testing through Playwright or Cypress would complement the existing white-box unit tests with workflow-level coverage. Finally, a mobile companion application targeting parents could extend the public score-viewing feature beyond a desktop browser context.

---

## Acknowledgements

The author thanks the administrative staff and leadership of MTs. Al-Hasyimiy, Bangil, Pasuruan, for their willingness to participate in interviews, observations, and iterative usability feedback throughout the development cycle. The author also expresses gratitude to academic supervisors for their guidance on the research design and to colleagues who reviewed early drafts of the system and the manuscript. The open-source software ecosystem — including Node.js, Express.js, SvelteKit, Drizzle ORM, SQLite, ExcelJS, Jest, and Electron.js — made this work possible.
