...
import { auditTable } from '../src/db/index.js';
...
async function saveAuditLog(req, res, responseData) {
	try {
		...
		// Try req.user (from VERIFY_TOKEN_MIDDLEWARE middlewares)
		if (req.user?.username) {
			userId = req.user.username;
		}
		// If Login action, get from response
		else if (action === 'User Login' && responseData?.user?.username) {
			userId = responseData.user.username;
		}
		// If Login action, try request body (Fallback)
		else if (action === 'User Login' && req.body?.username) {
			userId = req.body.username;
		}
		// Try to decode token from header
		...
		// NOISE FILTERING
		// If the target is "General" AND the user is "anonymous"
		// This hides "anonymous viewed General" logs.
		if (userId === 'anonymous' && (target === 'General' || !target)) {
			return;
		}

		// VIEW ONCE PER SESSION LOGIC
...
function determineTarget(req) {
	...
	const pathSegments = req.path
		.split('/')
		.filter((p) => p && p !== 'api' && p !== 'routes' && !p.match(/^\d+$/));
