...
async function seed() {
	...
	try {
        ...
        const subject = await db.select().from(Subjects).where(eq(Subjects.name, 'Matematika')).limit(1);
        const subjectId = subject[0].id;

		// 5. Create ClassSubject (Link Class + Subject + Teacher)
		console.log('Creating ClassSubject...');
        // Check if link exists
        let csLink = await db.select().from(classSubject)
        ...
        let classSubjectId;
        try {
             const newCS = await db.insert(classSubject).values({
                classId: classId,
                subjectId: subjectId,
                teacherId: teacherId
            }).returning();
            ...
            // Likely unique constraint violation, fetch existing
             const existingCS = await db.select().from(classSubject)
